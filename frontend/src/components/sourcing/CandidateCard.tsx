...
export function CandidateCard({ candidate, onViewFounder, onOutreach }: Props) {
  ...
  return <article className="panel group rounded-lg p-5 transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_45px_rgba(65,90,125,.13)]">
    ...
    <div className="mt-4 flex flex-wrap gap-1.5">{(profile?.source_types ?? Object.keys(candidate.handles ?? {})).map((tag) => <span key={tag} className="rounded-full bg-white/75 px-2.5 py-1 text-[10px] font-medium text-muted shadow-sm">{formatPredicate(tag)}</span>)}{websiteUrl && <SafeLink href={websiteUrl} onClick={(event) => event.stopPropagation()} className="inline-flex items-center gap-1 rounded-full bg-accent-soft px-2.5 py-1 text-[10px] font-bold text-accent">Website <ExternalLink className="h-3 w-3" /></SafeLink>}</div>
