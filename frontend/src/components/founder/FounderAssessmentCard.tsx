...
export function FounderAssessmentCard({ assessment, profile, history }: { assessment: FounderAssessment; profile: FounderProfile; history: number[] }) {
  ...
  const ratingStyle = assessment.rating === "Bullish" ? "bg-[#e4f2ed] text-success" : assessment.rating === "Bearish" ? "bg-[#fbe8e9] text-danger" : "bg-[#fff1df] text-warn";
  ...
  return <article className="panel overflow-hidden rounded-lg"><div className="p-5"><div className="flex items-start justify-between gap-3"><span className={`flex h-10 w-10 items-center justify-center rounded-md ${details.soft}`}><Icon className="h-5 w-5" /></span><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${ratingStyle}`}>{assessment.rating}</span></div><h3 className="mt-4 text-base font-bold">{assessment.title}</h3><p className="mt-1 text-xs leading-5 text-muted">{details.description}</p><div className="mt-4 rounded-md bg-surface-2/80 p-3 shadow-inner shadow-white/80"><div className="data-label">Why this rating</div><p className="mt-1.5 text-xs font-medium leading-5 text-ink-2">{assessment.body}</p></div><div className="mt-4 space-y-3 rounded-md bg-white/55 p-3"><InsightRow kind="support" label="Supporting evidence" text={explanation.support} meta={explanation.supportMeta} /><InsightRow kind="risk" label="Counter-evidence / risk" text={explanation.risk} /><InsightRow kind="unknown" label="Still unknown" text={explanation.unknown} /></div></div><div className="bg-gradient-to-br from-surface-2 to-[#edf4f4] px-5 pb-4 pt-4"><div className="mb-2 flex items-center justify-between"><div><div className="data-label">Trend · last 5 updates</div><div className="mt-1 flex items-center gap-1.5 text-xs font-bold" style={{ color: details.color }}><TrendIcon className="h-3.5 w-3.5" />{assessment.trend}</div></div><div className="text-right"><div className="numeric text-xl font-bold leading-none">{values.at(-1) ?? "—"}</div><div className="mt-1 text-[10px] text-muted">Current signal</div></div></div>{values.length ? <TrendChart values={values} color={details.color} /> : <div className="flex h-[92px] items-center justify-center text-xs text-muted">No measured signal yet.</div>}<div className="mt-1 flex justify-between text-[10px] text-muted-2"><span>Earlier evidence</span><span>Latest</span></div><div className="mt-3 rounded-md bg-white/55 p-3"><div className="data-label mb-1.5 flex items-center justify-between"><span>Assessment confidence</span><span className="numeric">{assessment.confidence}%</span></div><div className="h-1.5 bg-surface-3"><div className="h-full" style={{ width: `${assessment.confidence}%`, backgroundColor: details.color }} /></div><p className="mt-2 text-[10px] leading-4 text-muted">Each point is a versioned Memory update based on new evidence or an investor correction.</p></div></div></article>;
}
