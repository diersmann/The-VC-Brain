...
import { generateCandidateMemo, invalidateCandidateQueries, useCandidate, useCandidateMemo } from "../api/candidates";
...
export function DecisionDetailPage() {
  ...
  const { data: memo, error: memoError, isLoading: memoLoading, refetch: refetchMemo } = useCandidateMemo(founderId, candidate?.opportunity?.id);
  ...
  useEffect(() => {
    ...
    if (memoJobStatus === "succeeded") setMemoGenState("idle");
    else if (memoJobStatus === "degraded" || memoJobResultStatus === "degraded") setMemoGenState("degraded");
    else setMemoGenState("error");
    void invalidateCandidateQueries(queryClient, founderId, { includeMemo: true, jobId: memoJobId ?? undefined }).catch(() => undefined);
  }, [founderId, memoJobId, memoJobResultStatus, memoJobStatus, queryClient]);
  ...
  const generateMemo = async () => {
    if (!founderId || !candidate.opportunity?.id) {
  ...
  return <div className="mx-auto max-w-[1240px] pb-28">
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[270px_minmax(0,1fr)]"><DecisionDetailSidebar profile={profile} candidate={candidate} meta={meta} /><div className="min-w-0 space-y-5"><AiSummary profile={profile} meta={meta} /><DealMetrics profile={profile} meta={meta} /><AxisScreening profile={profile} /><InvestmentMemo candidate={candidate} memo={memo} memoError={memoDisplayError} memoLoading={memoLoading} memoGenState={memoGenState} onGenerate={generateMemo} onRetryMemo={() => { void refetchMemo(); void memoJob.refetch(); }} /><div className="grid gap-5 xl:grid-cols-2"><ListCard title="Key risks" eyebrow="Downside case" icon={AlertTriangle} items={meta.risks} tone="amber" /><ListCard title="Conditions to proceed" eyebrow="Next diligence" icon={CheckCircle2} items={meta.conditions} tone="blue" /></div><EvidenceCard profile={profile} /></div></div>
    <DecisionActionDock candidateId={candidate.id} opportunityId={candidate.opportunity?.id ?? null} currentState={candidate.opportunity?.lifecycle_state ?? "No opportunity state"} onSaved={() => undefined} />
  </div>;
}
