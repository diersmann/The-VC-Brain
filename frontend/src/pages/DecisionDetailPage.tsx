...
export function DecisionDetailPage() {
  ...
  useEffect(() => {
    if (memo?.status === "succeeded" || memo?.status === "failed" || memo?.status === "degraded") setMemoGenState("idle");
  }, [memo?.status]);
  ...
  return <div className="mx-auto max-w-[1240px] pb-28">
    ...
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><Link to="/decisions" className="inline-flex items-center gap-2 text-xs font-bold text-muted transition-colors hover:text-ink"><ArrowLeft className="h-4 w-4" /> Back to decision queue</Link><div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.1em] text-muted"><Clock3 className="h-3.5 w-3.5 text-amber-600" /> Decision due in {meta.deadline}</div></div>
    <div className="grid items-start gap-5 lg:grid-cols-[270px_minmax(0,1fr)]"><DecisionDetailSidebar profile={profile} candidate={candidate} meta={meta} /><main className="min-w-0 space-y-5"><AiSummary profile={profile} meta={meta} /><DealMetrics profile={profile} meta={meta} /><AxisScreening profile={profile} /><InvestmentMemo memo={memo} memoError={memoError} memoLoading={memoLoading} memoGenState={memoGenState} onGenerate={generateMemo} onRetryMemo={() => void refetchMemo()} /><div className="grid gap-5 xl:grid-cols-2"><ListCard title="Key risks" eyebrow="Downside case" icon={AlertTriangle} items={meta.risks} tone="amber" /><ListCard title="Conditions to proceed" eyebrow="Next diligence" icon={CheckCircle2} items={meta.conditions} tone="blue" /></div><EvidenceCard profile={profile} /></main></div>
