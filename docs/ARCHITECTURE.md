...
- Non-successful HTTP and request-validation responses use a versioned v1 error envelope while retaining the legacy `detail` key: `{version: "v1", detail, error: {version: "v1", code, message, retryable, request_id}}`. Codes are stable status classifications (`not_found`, `conflict`, `validation_error`, `service_unavailable`, and related request/server codes), and `retryable` is true only for bounded transient statuses (429 and 5xx, plus timeout classes). The generated opaque request ID is also returned as `X-Request-ID`; authored HTTP exception headers such as `Allow` and `Retry-After` remain intact. Validation details retain location/type but redact submitted values, context, and free-form validator messages. All 5xx details are generic to prevent exception or PII leakage. Legacy clients can continue reading `detail`; new clients should prefer the v1 metadata under `error`.
...
- Durable `job_runs` IDs/status/result/error metadata now track discovery (including periodic active-thesis fan-out), collection, inbound PDF parsing, processing, research, scoring, memo, avatar, and identity queue entry points; status reads expose `updated_at`, attempt counts, and terminal states for bounded polling. Inbound submission creates the parsing run alongside its idempotent outbox envelope and returns the ID; the parser commits `running` before PDF extraction, records safe transient/rate-limit/permanent terminal metadata for missing snapshots, rejected PDFs, unexpected extraction, and persistence failures, fingerprints/reuses page/evidence observations across downstream retries, and keeps downstream processing failures retryable through the separate processing-session handoff before final success. Collection, processing, research, scoring, memo, avatar, identity, and parsing workers reopen failed runs with an incremented attempt and no-op duplicate delivery after succeeded/cancelled runs (the client also recognizes model-supported cancellation); collection/avatar/processing/identity/parsing worker-created IDs are logged for legacy tasks that omit a job ID, while API-triggered identity resolution, processing, avatar batch, and inbound submission responses expose their IDs. Processing, identity, and parsing failures retain shared transient/rate-limit/permanent classification. Memo degradation is stored in `result.status` while the JobRun remains failed. API and periodic queue-entry failures are persisted as failed runs; dispatcher enqueue failures requeue/refund and leave their runs queued. A single transaction spanning parsing and downstream processing, lease-based crash recovery, generic cancel/retry orchestration, and atomic DB/Redis delivery remain follow-up work.
...
All synchronous Tavily discovery SDK calls run through a worker thread so the
async worker event loop is not blocked. LinkedIn, podcast, and hackathon
connectors remain discovery/delegation wrappers and never fetch those sites
directly; generic web collection owns the HTTP, redirect, SSRF, and byte-limit
guardrails. Generic HTTP requests disable ambient proxy environment variables,
and URL validation (including DNS resolution) runs before both Tavily Extract
and raw fetches. Tavily extraction is worker-thread offloaded with a bounded
15-second wait and the configured content-size cap; DNS pinning/TOCTOU
...
processes create the configured bucket concurrently. Live clean-start/restart
